...
class Teacher_logits(Dataset):
    def __init__(self, top_k_path='./logits.pt',targt_path='./target.pt', k = 100, teacher_vocab_size=128256, max_new_tokens = 100, default_value = -np.inf, device = 0):
        warnings.filterwarnings("ignore", message="You are using torch.load with weights_only=False*")
        torch.set_warn_always(False)
        self.device = f"cuda:{device}" if torch.cuda.is_available() else "cpu"
        self.dataset = torch.load(top_k_path,  map_location="cpu")
        self.k_value = k
        self.teacher_vocab_size = teacher_vocab_size
        self.max_seq_len = max_new_tokens
        self.default_value = default_value
        self.prompts = list(self.dataset.keys())
        self.target = torch.load(targt_path,  map_location="cpu")
