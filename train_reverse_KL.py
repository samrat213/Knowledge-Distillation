...
parser.add_argument("--run", default="llama_new_student_logits_reverse_KL", type=str, required=False)
...
class KnowledgeDistillation:
    ...
    def train_step(self, batch, task, alpha=0.5):
        ...
        for i in range(len(prompts)):
            input_str.append(prompts[i]+targets[i])
    ...
    def validation_loss(self, test_data, writer, epoch):
        ...
        for task, examples in test_data.items():
            ...
            for example in tqdm(examples, desc=f"Evaluating {task}"):
                ...
                inputs = self.tokenizer(prompt+reference,
                                    return_tensors="pt", 
                                    padding=True,
                                    truncation=True,
...
for epoch in range(epochs):
    ...
    for batch in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
        ...
        for task, examples in task_examples.items():
            ...
            batch_kd_loss += kd_loss.item()*num_examples
            batch_task_loss += task_loss.item()*num_examples
        
        writer.add_scalar('batch_loss', batch_loss, step)
        writer.add_scalar('batch_kd_loss', batch_kd_loss, step)
        writer.add_scalar('batch_task_loss', batch_task_loss, step)
        step+=1
