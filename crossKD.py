...
from teacher import Teacher_logits
import numpy as np
...
class KnowledgeDistillation:
    ...
    def compute_kd_loss(self, student_logits, teacher_logits, temperature=TEMP, attention_mask=None):

        """
        Compute knowledge distillation loss using Wasserstein distance
        ...
        Returns:
            Wasserstein distance based knowledge distillation loss
        """
        ...
        # Check and handle vocabulary size mismatch
        if student_logits.shape[-1] != teacher_logits.shape[-1]:
            ...
            # Alternative approach would be to project the smaller vocab to the larger one
            # but this would require additional parameters and training
    
        # Both logits should have the same sequence length
        if student_logits.shape[1] != teacher_logits.shape[1]:
        ...
        soft_student = F.softmax(student_logits / temperature, dim=-1)  # Not log_softmax for Wasserstein
    
        # Compute Wasserstein distance (Earth Mover's Distance)
        # For 1D distributions, this is efficiently computed using the cumulative distributions
        def wasserstein_distance(p, q):
        # Compute cumulative distributions
            p_cdf = torch.cumsum(p, dim=-1)
            q_cdf = torch.cumsum(q, dim=-1)
        
            # L1 distance between CDFs is the Wasserstein distance for 1D distributions
            return torch.abs(p_cdf - q_cdf).sum(dim=-1)
    
        # Apply the Wasserstein distance
        if attention_mask is not None:
            # Apply mask to focus on non-padding tokens
            wasserstein_losses = wasserstein_distance(soft_student, soft_teacher)
        
            # Apply the mask
            mask = attention_mask.float()
            masked_losses = wasserstein_losses * mask
        
            # Average over non-padded tokens
            w_loss = masked_losses.sum() / (mask.sum() + 1e-8)
        else:
        # Calculate for all tokens and average
            wasserstein_losses = wasserstein_distance(soft_student, soft_teacher)
            w_loss = wasserstein_losses.mean()
    
        # Scale by temperature squared as in KD
        return w_loss * (temperature ** 2)
...
teacher_output = Teacher_logits(top_k_path='./logits.pt',device=3)
epochs = 10
